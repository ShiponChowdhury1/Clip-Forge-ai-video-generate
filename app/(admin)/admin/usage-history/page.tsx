    ...
    <div>
            ...
            <tbody>
              {isLoading ? (
                ...
                entries.map((entry) => {
                  ...
                  return (
                    <tr key={entry.id} className="border-b border-[#1A3155]/50 hover:bg-[#1A2332]/40 transition-colors">
                      {/* User */}
                      ...
                      {/* Reference ID */}
                      <td className="py-4 px-5">
                        <span className="text-gray-400 font-mono text-sm">{entry.reference_id}</span>
                      </td>
                      {/* Status */}
                      <td className="py-4 px-5">
                        <span
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#111827] rounded-2xl w-full max-w-sm mx-auto shadow-2xl border border-[#1E293B] p-6">
            <button onClick={() => setDeleteTarget(null)} className="absolute top-4 right-4 text-gray-400 hover:text-white transition-colors">
              <X className="w-5 h-5" />
            </button>
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-lg font-bold text-white mb-2">Delete Log Entry</h3>
              <p className="text-gray-400 text-sm mb-6">
                Are you sure you want to delete the log for <span className="text-white font-medium">{deleteTarget.name}</span>? This action cannot be undone.
              </p>
              <div className="flex items-center gap-3 w-full">
                <button
                  onClick={() => setDeleteTarget(null)}
                  className="flex-1 px-4 py-2.5 rounded-lg bg-[#1A2332] border border-[#1A3155] text-sm text-gray-300 hover:text-white hover:border-[#2563EB] transition-colors"
                >
                  Cancel
                </button>
                <button
